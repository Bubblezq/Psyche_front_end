                  ...
                  <a-card class="ant-pro-pages-list-projects-card" hoverable :loading="loading"
                    ...
                    <a-card-meta :title="item.title" style="text-align: center">
                      <template slot="description">
                        <ellipsis :length="70">{{ item.description | ellipsis }}</ellipsis>
                      </template>
                    </a-card-meta>
                    <a-button type="primary" class="cardItemContent" ghost=true>
                      <router-link :to="{ name: 'TestDetail', params:{ id: item.id,title:item.title} }">开始测试</router-link>
                    </a-button>
                  </a-card>
                </a-list-item>
              </a-list>

              <a-pagination @change="onChange" :current="current" :total="totalPage" style="text-align: center"/>
...
<script>
  ...
  import {getTestList} from '@/api/psytest'
  ...
  export default {
    ...
    methods: {
      onChange(current) {
        ...
        getTestList(this.current).then(response => {
          console.log('点击输出', response.data)
      ...
      getList() {
        getTestList({'pageNum':this.current}).then(response => {
